 ...
 */
let runner = ({root, tests, fixtures, results, failures}, cb) => {
  let files   = tests.join(' ');
  let phantom = path.join(__dirname, '..', 'node_modules/phantomcss');
  let command = `casperjs test ${tests} --verbose --pre=${__dirname}/pre.js --includes=${__dirname}/includes.js --post=${__dirname}/post.js --root=${root} --tests=${files} --fixtures=${fixtures} --results=${results} --failures=${failures} --phantom=${phantom}`;
